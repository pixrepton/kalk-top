...
foreach ($topinstal_workflow_config_candidates as $topinstal_workflow_config_path) {
    if (is_file($topinstal_workflow_config_path)) {
        require_once $topinstal_workflow_config_path;
        $topinstal_workflow_config_loaded_from = $topinstal_workflow_config_path;
        break;
    }
}

$GLOBALS['topinstal_offer_workflow_config_bootstrap'] = array(
    'loadedFrom' => $topinstal_workflow_config_loaded_from,
    'candidates' => $topinstal_workflow_config_candidates,
    'classAvailable' => class_exists('TopInstal_MailIngressWorkflowConfig'),
);
...
if (!class_exists('TopInstal_MailIngressWorkflowConfig')) {
    class TopInstal_MailIngressWorkflowConfig {
        ...
        const GENERATOR_DEFAULT_ENDPOINT = 'https://www.topinstal.com.pl/pdf/wp-json/topinstal/v1/offer-documents/generate';
        ...
        public function resolve_generator_endpoint() {
            ...
            return array(
                'value' => self::GENERATOR_DEFAULT_ENDPOINT,
                'rawValue' => self::GENERATOR_DEFAULT_ENDPOINT,
                'configType' => 'endpoint',
                'route' => self::GENERATOR_ROUTE,
                'source' => 'default',
                'sourceName' => 'repo-default-generator-endpoint',
            );
        }
        ...
        private function get_string_bundle($option_name, $const_name, $default = '') {
            if ($const_name !== '' && defined($const_name)) {
                ...
                if (is_string($value) && trim($value) !== '') {
                    return array('value' => trim($value), 'source' => 'constant', 'sourceName' => $const_name);
                }
            }
            if ($const_name !== '') {
                $env = getenv($const_name);
                if (is_string($env) && trim($env) !== '') {
                    return array('value' => trim($env), 'source' => 'env', 'sourceName' => $const_name);
                }
            }
            if ($option_name !== '' && function_exists('get_option')) {
                $option = get_option($option_name, '');
                if (is_string($option) && trim($option) !== '') {
                    return array('value' => trim($option), 'source' => 'option', 'sourceName' => $option_name);
                }
            }
            return array('value' => (string) $default, 'source' => 'default', 'sourceName' => 'default');
        }
